...
def dh2A(dh: list[float], jt: str) -> Callable[[float], NDArray]:
    ...
    if jt == 'r':
        ...
        def A(q: float) -> NDArray:
            ...
            return np.array([[cth, -sth*cal, sth*sal, a*cth],
...
class SerialArm:
    ...
    def __init__(self, dh: list[list[float]], jt: list[str]|None=None,
        ...
        for i in range(self.n):
            # TODO use the function definition above (dh2A), and the dh parameters and joint type to
    ...
    def fk(self, q: Iterable[float], index: int|Iterable[int]|None=None,
        ...
        if base is not None and start_frame == 0:
            T = self.base
        else:
            T = np.eye(4)
    ...
    def ik_position(self, target: NDArray, q0: list[float]|NDArray|None=None,
        ...
        if debug == True:
            ...
            viz.add_arm(arm, joint_colors=[[1.0, 51.0/255.0, 1.0, 1]]*arm.n)
        ...
        while np.linalg.norm(error) > tol and iters < max_iter:
            ...
            if method == 'J_T':
                ...
                raise ValueError("Invalid method. Must be 'J_T' or 'pinv'")
            ...
            if debug==True:
                ...
                if debug_step == True:
                    ...
                    viz.hold(0.5)
                print(f'error: {np.linalg.norm(error):.4f} \t iter: {iters}')
