    ...
    def ik_position(self, target: NDArray, q0: list[float]|NDArray|None=None,
        ...
        while np.linalg.norm(error) > tol and iters < max_iter:
            ...
            if method == 'pinv':
                ...
                J = self.jacob(q)
                J_pos_and_orientation = J[0:6, :]  # Extract all of Jacobian
                J_pseudo_inv = np.linalg.pinv(J_pos_and_orientation)
                ...
                J = self.jacob(q)
                J_pos_and_orientation = J[0:6, :]  # Extract all of Jacobian
                J_transpose = J_pos_and_orientation.T
