...
# Pen extends 15cm downward from wrist mounting point
tip_rot = tr.roty(-np.pi/2)
pen_offset = np.array([0, 0, 0.15])  # Pen extends 15cm down
tip = tr.se3(tip_rot, pen_offset)
...
signature_trajectory = [
    ...
    (np.array([1.1, -0.25, 0.15]), R_des, True),   # Draw vertical line up
    (np.array([1.15, -0.23, 0.15]), R_des, True),  # Curve top right
    (np.array([1.18, -0.20, 0.10]), R_des, True),  # Continue curve
    ...
    (np.array([1.2, -0.15, 0.05]), R_des, False),  # Move to next letter
    
    # Letter "a" - small circle with tail
    (np.array([1.2, -0.15, 0.03]), R_des, True),   # Lower pen (mid height)
    (np.array([1.18, -0.13, 0.03]), R_des, True),  # Small circle top
    (np.array([1.18, -0.13, 0.0]), R_des, True),   # Circle right side
    (np.array([1.2, -0.15, 0.0]), R_des, True),    # Circle bottom
    (np.array([1.22, -0.13, 0.03]), R_des, True),  # Right tail up
    
    # Letter "v" - continuous from "a"
    (np.array([1.25, -0.10, 0.0]), R_des, True),   # Left stroke down
    (np.array([1.27, -0.08, 0.03]), R_des, True),  # Right stroke up
    
    # Letter "i" - dot will be separate
    (np.array([1.30, -0.05, 0.0]), R_des, True),   # Vertical stroke down
    (np.array([1.30, -0.05, 0.05]), R_des, True),  # Stroke up
    
    # Pen lift for dot
    (np.array([1.30, -0.05, 0.08]), R_des, False), # Lift for dot
    (np.array([1.30, -0.05, 0.08]), R_des, True),  # Dot (brief touch)
    (np.array([1.30, -0.05, 0.05]), R_des, False), # Lift again
    
    # Move to "s"
    (np.array([1.32, -0.02, 0.05]), R_des, False), # Move to S
    (np.array([1.32, -0.02, 0.03]), R_des, True),  # Lower pen
    
    # Letter "s" - snake curve
    (np.array([1.33, 0.0, 0.05]), R_des, True),    # Upper curve right
    (np.array([1.34, 0.0, 0.03]), R_des, True),    # Curve back
    (np.array([1.35, 0.02, 0.0]), R_des, True),    # Lower curve left
    (np.array([1.36, 0.02, 0.0]), R_des, True),    # End stroke
    
    # Final pen lift
    (np.array([1.36, 0.02, 0.08]), R_des, False),  # Lift pen off paper
]
...
# -----------------------------
for pose_idx in range(len(desired_poses)):
    p_des, R_des = desired_poses[pose_idx]
    pen_down = pen_states[pose_idx]
    ...
    for iter in range(300):
        ...
        # Check if converged
        if np.linalg.norm(pos_err) < 0.01 and abs(1.0 - x_align) < 0.01:
...
print(f"Total waypoints: {len(desired_poses)}")
print(f"Drawing waypoints: {sum(pen_states)}")
print(f"Movement waypoints: {len(pen_states) - sum(pen_states)}")
print("="*60)
