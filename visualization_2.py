...
light_grey = np.array([0.7, 0.7, 0.7, 1])
...
class ArmMeshObject:
    def __init__(self, arm, link_colors=None, joint_colors=None, q0=None,
        ...
        if link_colors is None:
            link_colors = [light_grey] * self.n

        if joint_colors is None:
            joint_colors = [grey] * self.n  # Dark grey for all joints
        ...
        joint_width = np.max([arm_scale, 0.10])*0.20
        joint_height = np.max([arm_scale, 0.10])*0.15  # Half the original depth
...
class LinkMeshObject:
    def __init__(self, dh, jt='r',
        ...
        # Create cylindrical joint mesh with closed ends
        rows = 10
        cols = 20
        cylinder_mesh = gl.MeshData.cylinder(rows=rows, cols=cols, radius=[w/2, w/2], length=h)
        cylinder_verts = cylinder_mesh.vertexes()
        cylinder_faces = cylinder_mesh.faces().astype(np.int32)
        
        # Rotate cylinder to point along z-axis and center it
        cylinder_verts = cylinder_verts - np.array([0, 0, h/2])
        
        # Add center points for end caps
        top_center = np.array([[0, 0, h/2]])
        bottom_center = np.array([[0, 0, -h/2]])
        all_verts = np.vstack([cylinder_verts, top_center, bottom_center])
        
        top_center_idx = len(cylinder_verts)
        bottom_center_idx = len(cylinder_verts) + 1
        
        # Create cap faces - connect rim vertices to center points
        cap_faces = []
        # Top cap (first ring of vertices)
        for i in range(cols):
            next_i = (i + 1) % cols
            cap_faces.append([i, next_i, top_center_idx])
        
        # Bottom cap (last ring of vertices)
        base_idx = len(cylinder_verts) - cols
        for i in range(cols):
            next_i = (i + 1) % cols
            cap_faces.append([base_idx + next_i, base_idx + i, bottom_center_idx])
        
        all_faces = np.vstack([cylinder_faces, np.array(cap_faces, dtype=np.int32)]).astype(np.int32)
        ...
        self.joint_points = all_verts @ Rz @ Rx + v2
        self.joint_faces = all_faces

        if link_color is None:
            link_color = light_grey
        ...
        if joint_color is None:
            joint_color = grey  # Dark grey
        elif not isinstance(joint_color, (np.ndarray)):
            joint_color = np.array(joint_color)
        # Create enough colors for all cylinder faces
        num_joint_faces = len(self.joint_faces)
        self.joint_colors = np.zeros((num_joint_faces, 3, 4)) + joint_color

    @staticmethod
    def points_to_mesh(link_points, joint_points, joint_faces):
        ...
        # Use the cylinder faces to create proper joint mesh
        joint_mesh = joint_points[joint_faces]
    ...
    def get_mesh(self, R, p):
        ...
        return self.points_to_mesh(lp, jp, self.joint_faces)
...
class EEMeshObject:
    def __init__(self, scale=1, radius=0.0375):
        # Create a sphere for the end effector (1/4 original size)
        radius = radius * scale
        sphere_mesh = gl.MeshData.sphere(rows=20, cols=20, radius=radius)
        
        self.points = sphere_mesh.vertexes()
        self.faces = sphere_mesh.faces()
        
        # Dark blue color for the sphere
        num_faces = len(self.faces)
        self.colors = np.zeros((num_faces, 3, 4)) + dark_blue

    def get_mesh(self, R, p):
        # Transform sphere vertices to end effector position and orientation
        transformed_points = self.points @ R.T + p
        # Return mesh using the sphere faces
        return transformed_points[self.faces]
